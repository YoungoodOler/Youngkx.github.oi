...
- 首屏只显示大号 `Youngkx Blog` 标题和背景星尘；同一个标题随滚动缩小并迁移到第二阶段左侧。
- 进一步放大第二阶段的 `Youngkx Blog` 标题并略微下移；根据标题实际宽度动态计算横向位置，使其左边缘继续与下方引句内容栏对齐。
- 星球初始隐藏，滚动后从屏幕右上边缘沿非线性弧线旋转飞入。
- 引句、说明和操作入口拆为独立文字层，按顺序从下方带景深上浮；三层完整同屏停留后，再随继续滚动向上虚化退出。
- 两条轮播引句改为从左向右的横向扫入与滑出，不再重复整组文字的纵向模糊浮现。
- 首屏 `Youngkx Blog` 标题在缩放迁移过程中始终保持清晰；文章区、分类区及各内页大标题使用由模糊到清晰的景深浮入效果。
- 恢复亮色主题下首页文章卡片悬停时的透明化反馈，使背景星尘能够透过卡片显现。
- 移除点击时的大尺寸扩散圆环，只保留轻量粒子扰动和少量光尘反馈。
- 主题与页面切换改用独立全屏 Canvas 模拟原有星空色板：高密度星尘沿随机非线性轨迹高速扰动并形成短暂遮盖，界面更新后继续逸散消失。
- 提高切换扰流的星尘密度、曲线偏移、运动方向数量、粒子尺寸差异和拖尾强度，使遮盖过程更快速、更杂乱。
- 切换层不修改或重建主页原有 Three.js 背景粒子场，遮盖结束后直接显露原有交互星尘。
- 将 Three.js 交互星尘提升为全站背景：主页同时显示滚动飞入的星球，文章列表、分类、文章正文及其他页面仅显示粒子场。
- 恢复 Three.js 交互式宇宙粒子场，提供真实星空色温、呼吸闪烁、滚动漂移、鼠标涡流、移动光尘和轻量点击扰动。
- Three.js 场景在切换主题时原地混合颜色，不再销毁并重建 WebGL 场景。
- 手机端降低 Three.js 几何细分和设备像素比、关闭触摸粒子轨迹，但保留完整动画帧率与过渡过程。
