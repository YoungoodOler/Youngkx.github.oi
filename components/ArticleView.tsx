...
function detectLanguage(container: HTMLElement, codeElement: HTMLElement, code: string) {
  const classTokens = [codeElement, codeElement.parentElement, container].flatMap((element) =>
  ...
  if (/^(?:GET|POST|PUT|PATCH|DELETE|HEAD)\s+\S+\s+HTTP\/\d|^HTTP\/\d\.\d\s+\d{3}/m.test(source))
    return 'HTTP';
  if (/<(?:!doctype|html|head|body|main|section|div|span|a|p|script|style)\b/i.test(source))
    return 'HTML';
  if (
    /(?:^|\n)\s*#include\b|\bstd::|\bcout\s*<<|\bcin\s*>>|\bint\s+main\s*\(|\b(?:printf|scanf)\s*\(/.test(
      source,
    )
  )
    return /\bstd::|\biostream\b|\bcout\b|\bcin\b|\btemplate\s*</.test(source) ? 'C++' : 'C';
  if (/\b(?:using\s+System|Console\.WriteLine|namespace\s+\w+\s*\{)\b/.test(source)) return 'C#';
  if (/\b(?:interface|type)\s+\w+|:\s*(?:string|number|boolean)\b|\bas const\b/.test(source))
    return 'TypeScript';
  if (
    /\b(?:const|let|var)\s+\w+|=>|console\.(?:log|error|warn)\b|\bfunction\s+\w+\s*\(/.test(source)
  )
    return 'JavaScript';
  if (/(?:^|\n)\s*(?:def\s+\w+|from\s+\S+\s+import|import\s+\S+|print\s*\()/m.test(source))
    return 'Python';
  ...
  if (
    /\b(?:SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM)\b[\s\S]*\b(?:FROM|VALUES|SET|WHERE)\b/i.test(
      source,
    )
  )
    return 'SQL';
  ...
  if (/(?:^|\n)\s*(?:[.#][\w-]+|@media|[a-z][\w-]*)[^{]*\{[\s\S]*[\w-]+\s*:/i.test(source))
    return 'CSS';
...
export default function ArticleView({ article }: { article: ArticleRecord }) {
  ...
  const characters = useMemo(
    () => article.content.replace(/<[^>]+>/g, '').replace(/\s+/g, '').length,
    [article.content],
  );
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      const modernBlocks = [...root.querySelectorAll<HTMLPreElement>('pre')].filter(
        (pre) => !pre.closest('figure.highlight'),
      );
      ...
      legacyBlocks.forEach((figure) => {
        const codePre =
          figure.querySelector<HTMLPreElement>('.code pre') ??
  ...
  return (
    <main className="modern-article-page">
      ...
      <SiteHeader navClassName="article-nav-links" />

      <header className="modern-article-hero">
        <m.div
          className="shell"
          initial={{ opacity: 0, y: 78, filter: 'blur(20px)' }}
      ...
      <div className="modern-article-layout shell">
        <article
          ref={contentRef}
          className="modern-article-content"
          dangerouslySetInnerHTML={articleMarkup}
        />
        <div className="modern-copyright">
          <span>作者</span>
          <strong>Youngkx</strong>
          <span>说明</span>
          <code>原创内容，转载请注明出处</code>
        </div>
      </div>
      ...
      <div className="modern-article-tools">
        <Link href="/#top" title="返回主页" aria-label="返回主页">
          ⌂
        </Link>
        <button
        ...
        <button onClick={copyLink} title="复制链接" aria-label="复制链接">
          {copied ? '✓' : '⧉'}
        </button>
        <button
